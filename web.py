...
def get_temp_file_path(input_file):
    ...
    safe_name = re.sub(r"[^A-Za-z0-9._-]+", "_", base_name).strip("._") or "upload"
    digest = hashlib.sha256(input_file.getvalue()).hexdigest()[:12]
    return os.path.join(TEMP, f"{safe_name}-{digest}.ncm")
