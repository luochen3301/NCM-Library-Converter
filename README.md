...
![NCM Library Converter V2.2 图文预览](docs/readme-preview.svg)
...
![Version](https://img.shields.io/badge/Version-V2.2-35C2A4)
...
原项目提供 `.ncm` 音频文件转换能力，并包含 Windows 客户端与 Web 使用方式。本项目在保留 NCM 转换核心逻辑的基础上，重点扩展了桌面端音乐库管理体验：音乐库扫描、转换状态记忆、增量转换队列、历史日志、设置管理、深色主题、本地语言分类实验页、转换结果摘要和失败分组恢复。
...
| 转换摘要 | 每次队列结束后显示成功数、失败数、跳过数、耗时、输出位置，并提供打开输出、重试失败和导出日志操作。 |
| 失败分组 | 队列页会按权限、输出目录、源文件缺失、磁盘空间、格式异常等原因聚合失败项。 |
| 历史与日志 | 查看转换结果、失败信息，并可导出日志文本。 |
| 本地语言分类 | 实验页基于文件名和目录文本推断中文、英文、日文、韩文、混合、其他和未知类别。 |
...
1. 在 GitHub Releases 下载 `NCM-Library-Converter-V2.2-windows.zip`。
2. 解压压缩包。
...
- Qt offscreen 启动检查：确认默认 `obsidian` 主题和 5 个侧边栏页面可初始化
...
**转换失败后怎么处理？**  
V2.2 会在队列页按失败原因分组。你可以复制问题、定位源文件夹，或直接重试某一组失败文件。

**删除源 NCM 是否安全？**  
设置里有“成功后删除源 NCM”选项，但这是不可逆操作。建议确认输出文件无误并做好备份后再开启。

**语言分类会联网吗？**  
不会。V2.2 的语言分类实验功能只基于本地文件名和目录文本进行 Unicode 脚本推断。
...
- `VERSION` 更新为 `V2.2`
- `LICENSE` 保留在仓库根目录
- `RELEASE_NOTES_V2.2.md` 已更新
- `DISTRIBUTION_README_V2.2.txt` 已更新
- Windows 压缩包命名为 `NCM-Library-Converter-V2.2-windows.zip`
